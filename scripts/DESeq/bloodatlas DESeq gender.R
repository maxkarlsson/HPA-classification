...
BiocManager::install(c('S4Vectors', 'org.Hs.eg.db', 'ggthemr', 'GO.db'))
...
gene.info <- read_delim("./data/anno/ensembl_anno.tsv", delim='\t')
...
## blood counts
counts <- read_delim('./data/count data/blood_count/summarizedTPM_count_gene_level_rename.txt', delim = '\t')
pdata <- read_delim('./data/count data/blood_count/sample_info.txt', delim = '\t')
#counts.pseudo <- round(counts[,-1])
counts.pseudo <- 
  counts %>% 
